...
### Forgotten or mistaken password

With having a secured wifi network generated by the board, we figured it could cause some trouble if you forgot that password and wanted to update your board.
Therefore if you put the board into recovery mode by power-cycling (rebooting) it 3 times in 10 seconds, it will reset all settings.
See [/#recovering-an-owie-flash](Recovering an Owie flash) for more information on the behavior when Owie enters recovery mode.
You don't even need to connect to the recovery network, just waiting another 5-10 seconds (so the rolling restart window is empty) and power cycling your board again will reset everything.
