...
void testUninitializedGauge() {
  TEST_ASSERT_EQUAL(0, gauge->getBatteryPercentage());
...
void testSocAfterFirstVoltageMessageAndStateSaving() {
    ...
    TEST_ASSERT_EQUAL(53, gauge->getBatteryPercentage());
 ...
 * 2. Charges the board with 2 amps for an hour. Emulates the target voltage based SOC of 68%
...
void testSimpleChargeAndHalfwayDischarge() {
    ...
    TEST_ASSERT_EQUAL(18, gauge->getBatteryPercentage());
    ...
    TEST_ASSERT_EQUAL(68, gauge->getBatteryPercentage());
    ...
    TEST_ASSERT_EQUAL(43, gauge->getBatteryPercentage());
